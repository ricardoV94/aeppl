...
def test_loc_transform(scalar_loc, rv_size):
    ...
    loc_test_val = 4.0 if scalar_loc else np.full(rv_size, 4.0)
...
@pytest.mark.parametrize("rv_size", [None, 1, 2, (3, 1)])
def test_scale_transform(scalar_scale, rv_size):
    # Incompatible parametrization
    if not scalar_scale and rv_size is None:
        return
    ...
    scale_test_val = 4.0 if scalar_scale else np.full(rv_size, 4.0)
    y_test_val = np.full(rv_size, 1.0)
...
@pytest.mark.parametrize("rv_size", [None, 1, 2, (3, 2)])
def test_affine_transform(rv_size):
    loc = at.scalar("loc")
